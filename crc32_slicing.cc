...
Crc32Slicing::Crc32Slicing() { state = 0xffffffff; }
...
static inline uint32_t crc_process_iter(uint32_t crc, const uint32_t *current) {
#if !defined(_MSC_VER) && __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
  uint32_t one = *current++ ^ (((crc & 0xff) << 24) | ((crc & 0xff00) << 8) |
                               ((crc >> 8) & 0xff00) | ((crc >> 24) & 0xff));
  uint32_t two = *current;
  return Crc32Lookup[0][two & 0xFF] ^ Crc32Lookup[1][(two >> 8) & 0xFF] ^
  ...
  uint32_t two = *current;
  return Crc32Lookup[0][(two >> 24) & 0xFF] ^
...
void Crc32Slicing::update64(const void *buf, size_t len) {
  const char *src = (const char *)buf;
  for (size_t i = 0; i < len; i += 8)
    state = crc_process_iter(state, (uint32_t *)(src + i));
}

uint32_t Crc32Slicing::update_final(const void *buf, size_t len) {
  const uint32_t *src = (const uint32_t *)buf;
  for (; len >= 8; len -= 8) {
    state = crc_process_iter(state, src);
    src += 2;
  }
  const uint8_t *currentChar = (const uint8_t *)src;
  while (len--)
    state = (state >> 8) ^ Crc32Lookup[0][(state & 0xFF) ^ *currentChar++];
  return ~state;
}
